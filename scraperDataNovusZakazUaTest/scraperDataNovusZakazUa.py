...
def fetch_product_data_api(offset=0, limit=40):
...
def save_to_excel(data, filename='scraper_novus_kava_v_zernakh.xlsx'):
...
def fetch_and_save_data_api(filename='scraper_novus_kava_v_zernakh.xlsx', limit=40):
    """Основна функція для збору даних і збереження у файл."""
    offset = 0
    product_list = []

    while True:
        products = fetch_product_data_api(offset, limit)
        ...
        offset += limit  # Оновлюємо зсув
        print(f"[ЛОГ] Наступна сторінка: offset={offset}")

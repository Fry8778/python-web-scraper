...
def save_to_excel(data, filename='varus_products5.xlsx'):
...
def fetch_and_save_data_api(category, filename='varus_products5.xlsx'):
    ...
    while True:
        ...
        for product in products:
            ...
            # stock = product.get('stock', {}).get('qty', 0)
            # if stock == 0:
            ...
            # Отримуємо ціну та знижку
            price_data = product.get('price', {})
            price = extract_value(price_data.get('price', 0))
            special_price = extract_value(price_data.get('special_price', 0))
            discount = price_data.get('special_price_discount', 0)

            # Обчислення знижки, якщо є спеціальна ціна
            discount_str = f"-{discount}%" if discount else ''
            price_with_discount = special_price if discount else ''
